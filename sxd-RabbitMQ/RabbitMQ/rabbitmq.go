...
func (r *RabbitMQ) PublishSimple(message string) {
	...
	r.channel.Publish(
		...
		false,
		// 如果为true，当exchange发送消息到队列后发现队列上没有消费这，则会把消息返还给发送者
		false,
		amqp.Publishing{
...
func (r *RabbitMQ) ConsumeSimple() {
	...
	msgs, err := r.channel.Consume(
		...
		// 是否独有
		false, // exclusive
		// 设置为true, 标识 不能将同一个Connection中生产者发送的消息传递给这个connection中的消费者
		false, // no-local
		// 列是否阻塞
		false, // no-wait
		nil,   // args
	...
	log.Printf("[*] Waiting for message. To exit oress Ctrl+C")
	<-forever
}
...
// 匹配 sxd.* 标识匹配 sxd.hello, sxd.hello.one 需要用sxd.#才能匹配到
